...
static int adf702x_regs_set_pa_level(const struct device *dev, int8_t dBm)
{
	struct adf702x_context *ctx = dev->data;
	uint8_t dbm_lookup[] = {
		1, 2, 3, 4, 4, 5, 5, 6, 7, 8, 10, 11, 12,
		15, 18, 20, 22, 25, 27, 28, 31, 33, 35,
		37, 39, 41, 43, 45, 48, 50, 52, 54, 56, 58,
	};

	ctx->conf_regs.radio_pa_level = dbm_lookup[CLAMP(dBm, -20, 13) + 20];
...
static int adf702x_set_txpower(const struct device *dev, int16_t dBm)
{
	...
	return adf702x_configure_device(dev);
}
			    ...
			    struct ieee802154_attr_value *value)
{
	...
	case IEEE802154_ATTR_ADF702X_POWER:
		...
		LOG_INST_INF(conf->log, "PA_LEVEL readback: %02d", bram[0]);
		break;
	case IEEE802154_ATTR_ADF702X_RSSI:
		adf702x_read_rssi(dev, &rssi);
		LOG_INST_INF(conf->log, "RSSI: %d dBm", rssi);
		break;
